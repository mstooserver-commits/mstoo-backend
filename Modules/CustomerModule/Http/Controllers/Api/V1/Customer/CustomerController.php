...
class CustomerController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // return response()->json(response_formatter(DEFAULT_403), 401);
            ...
            if ($customer && class_exists(\Modules\ProMemberManagement\Services\ProMemberService::class)) {
                $pro = app(\Modules\ProMemberManagement\Services\ProMemberService::class);
                $payload['is_pro_member'] = $pro->isProMember($customer->id) ? 1 : 0;
                $payload['pro_membership'] = $pro->activeMembership($customer->id);
            }
    ...
    public function forgot_password(Request $request): JsonResponse
    {
        ...
        $identity = $request['phone_or_email'] ?? $request['email_or_phone'];
        DB::table('password_resets')->where('phone', $identity)->delete();
