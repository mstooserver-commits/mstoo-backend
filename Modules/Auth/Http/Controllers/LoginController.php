...
class LoginController extends Controller
{
    ...
    public function admin_login(Request $request): RedirectResponse
    {
        ...
        if (isset($user)) {
            ...
            if ($remaining > 0) {
                Toastr::error(translate('Your account is temporarily blocked. Please_try_again_after_'). CarbonInterval::seconds($remaining)->cascade()->forHumans());
                return back();
            }
        ...
        if (isset($user) && Hash::check($request['password'], $user->getAuthPassword())) {
            $hasActiveRole = $user->is_active && (
