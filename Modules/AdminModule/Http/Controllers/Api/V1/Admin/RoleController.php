...
class RoleController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            if ($request->filled('permissions')) {
                sync_role_access_flags($role, filter_assignable_permissions((array)$request['permissions']));
            }
            $role->save();
        });
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $role = $this->role->where(['id' => $id])->first();
        if (!$role) {
            return response()->json(response_formatter(DEFAULT_204), 404);
        }
        if ($role->is_system && auth()->user()->user_type !== 'super-admin') {
            return response()->json(response_formatter(ACCESS_DENIED), 403);
        }
    ...
    public function destroy(Request $request): JsonResponse
    {
        ...
        $roles = $this->role->withCount('users')->whereIn('id', $request['role_ids'])->get();
        $deletable = $roles->filter(function ($role) {
            return !$role->is_system && $role->users_count < 1;
        });
        if ($deletable->count() > 0) {
            $this->role->whereIn('id', $deletable->pluck('id'))->delete();
