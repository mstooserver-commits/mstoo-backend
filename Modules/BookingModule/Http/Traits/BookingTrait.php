...
trait BookingTrait
{
    protected function place_booking_request($user_id, $request, $transaction_id)
    {
        ...
        foreach ($cart_data->pluck('sub_category_id')->unique() as $sub_category) {
            ...
            DB::transaction(function () use ($sub_category, $booking, $transaction_id, $request, $cart_data, $user_id) {
                ...
                $fullCart = Cart::where(['customer_id' => $user_id])->get();
                $proAdj = class_exists(\Modules\ProMemberManagement\Services\ProMemberService::class)
                    ? app(\Modules\ProMemberManagement\Services\ProMemberService::class)->cartAdjustments($user_id, $fullCart)
                    : ['pro_discount' => 0, 'service_fee' => 0];
                $portion = class_exists(\Modules\ProMemberManagement\Services\ProMemberService::class)
                    ? app(\Modules\ProMemberManagement\Services\ProMemberService::class)->allocateToPortion(
                        $proAdj,
                        (float)$cart_data->sum('total_cost'),
                        (float)$fullCart->sum('total_cost')
                    )
                    : ['pro_discount' => 0, 'service_fee' => 0];
                $booking->total_pro_discount_amount = $portion['pro_discount'];
                $booking->service_fee = $portion['service_fee'];
                $booking->total_booking_amount = round((float)$cart_data->sum('total_cost') - $portion['pro_discount'] + $portion['service_fee'], 2);
    ...
    {
        $service_cost = $booking['total_booking_amount']
            - $booking['total_tax_amount']
            + $booking['total_discount_amount']
            + $booking['total_campaign_discount_amount']
            + $booking['total_coupon_discount_amount']
            + (float)($booking['total_pro_discount_amount'] ?? 0)
