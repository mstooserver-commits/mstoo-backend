...
class BookingController extends Controller
{
    ...
    public function request_accept(Request $request, string $booking_id): JsonResponse
    {
        ...
        $providerId = optional($request->user()->provider)->id ?? $request->user()->id;
        $booking = $this->booking->where('id', $booking_id)
    ...
    public function request_reject(Request $request, string $booking_id): JsonResponse
    {
        ...
        $booking = $this->booking->where('id', $booking_id)->where('provider_id',$request->user()->id)->first();
        if (isset($booking)) {
            if (!setting_flag(business_live('provider_can_cancel_booking', 'service_setup', 0))) {
                return response()->json(response_formatter(BOOKING_CANCEL_DISABLED_403), 403);
            }
            $booking->provider_id = $request->user()->id;
