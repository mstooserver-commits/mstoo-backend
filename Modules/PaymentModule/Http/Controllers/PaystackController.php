...
class PaystackController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (!is_null($config) && $config->mode == 'live') {
            $values = (array) $config->live_values;
        } elseif (!is_null($config) && $config->mode == 'test') {
            $values = (array) $config->test_values;
        }

        if (!empty($values)) {
            $config = array(
                'publicKey' => env('PAYSTACK_PUBLIC_KEY', $values['public_key'] ?? null),
                'secretKey' => env('PAYSTACK_SECRET_KEY', $values['secret_key'] ?? null),
                'paymentUrl' => env('PAYSTACK_PAYMENT_URL', $values['callback_url'] ?? $values['payment_url'] ?? null),
                'merchantEmail' => env('MERCHANT_EMAIL', $values['merchant_email'] ?? null),
            );
