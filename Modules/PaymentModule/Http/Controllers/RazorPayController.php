...
class RazorPayController extends Controller
{
    ...
    public function index(Request $request): View|Factory|JsonResponse|Application
    {
        ...
        if (is_null($request['post_id'])) {
            ...
            // custom code by naresh to add commission to service cart amount
            $default_commission = business_config('default_commission', 'business_information')->live_values;
            if(isset($default_commission)){
                $commission_amount = ($default_commission * $order_amount)/100;
                $order_amount = $order_amount + $commission_amount;
            }
