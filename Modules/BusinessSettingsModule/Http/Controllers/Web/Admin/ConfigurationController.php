...
class ConfigurationController extends Controller
{
    ...
    public function third_party_config_set(Request $request): JsonResponse
    {
        ...
        if ($request['party_name'] == 'google_map') {
            ...
            $existingPush = $this->business_setting->where(['key_name' => 'push_notification', 'settings_type' => 'third_party'])->first();
            $submittedKey = trim((string) $request->input('server_key', ''));
            if ($submittedKey === '' || preg_match('/^[•*]{6,}$/u', $submittedKey)) {
                $request->merge(['server_key' => data_get($existingPush, 'live_values.server_key', '')]);
            }
            $additional_data = [
                'server_key' => 'required'
            ];
     ...
     * @return Application|Factory|View
     */
    public function get_customer_settings(Request $request)
    {
    ...
    public function set_customer_settings(Request $request): RedirectResponse
    {
        ...
        admin_audit('business_settings.updated', 'customer_config', ['page' => $request['web_page'] ?? null]);
        Toastr::success(DEFAULT_UPDATE_200['message']);
        return back();
    }
}
