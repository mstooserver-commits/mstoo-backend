...
class CartController extends Controller
{
    ...
    public function list(Request $request): JsonResponse
    {
        ...
        $proAdjustments = class_exists(\Modules\ProMemberManagement\Services\ProMemberService::class)
            ? app(\Modules\ProMemberManagement\Services\ProMemberService::class)->cartAdjustments($request->user()->id)
            : ['pro_discount' => 0, 'service_fee' => 0, 'is_pro_member' => false];

        return response()->json(response_formatter(DEFAULT_200, [
            'cart' => $cart,
            'wallet_balance' => with_decimal_point($wallet_balance),
            'pro_member' => $proAdjustments,
            'payable_total' => cart_total($request->user()->id),
        ]), 200);
    }
